...
<div id="mainarea">
    ...
    <div class="maincontent">
        ...
        <div class="tag_cloud" id="list_users">
            <table width="100%">
                <tr>
                    <th width="30%">Nom</th><th width="30%">Login</th><th width="20%">Groupe</th><th width="10%">Cnx</th><th width="10%">Vote</th>
                </tr>
                <!-- BEGIN users -->
                <tr id="user_{users.ID}" onclick="javascript:getUser({users.ID});">
                    <!-- BEGIN user -->
                    <td>{users.user.NAME}</td><td>{users.user.LOGIN}</td><td>{users.user.GROUP_NAME}</td><td>{users.user.LAST_CONNECTION}</td><td>{users.user.LAST_BET}</td>
                    <!-- END user -->
                    <!-- BEGIN admin -->
                    <td><b>{users.admin.NAME}</b></td><td>{users.admin.LOGIN}</td><td>{users.admin.GROUP_NAME}</td><td>{users.admin.LAST_CONNECTION}</td><td>{users.admin.LAST_BET}</td>
                    <!-- END admin -->
                </tr>
                <!-- END users -->
            </table>
        ...
        <div class="tag_cloud">
            <form name="add_user" action="/?act=add_user" method="post">
                ...
                <table>
                    <tr><td width="40%">Login :</td><td width="40%">Pass :</td><td width="20%">&nbsp;</td></tr>
                    <tr><td><input type="text" size="25" name="login" id="login" /></td><td><input type="text" size="25" name="pass" id="pass" /></td><td>&nbsp;</td></tr>
                    <tr><td>Nom :</td><td>Groupe :</td><td>Admin :</td></tr>
                    <tr><td><input type="text" size="25" name="name" id="name" /></td>
                        ...
                        <td style="text-align:center;"><input type="checkbox" name="admin" id="admin" value="1"/></td></tr>
                    <tr><td colspan=3>Email :</td></tr>
                    <tr><td colspan=3><input type="text" size="50" name="email" id="email" /></td></tr>
                    <tr>
                        <td colspan=2 style="text-align:center;">
                            <input type="button" name="add_user" id="add_user" onclick="saveUser()" value="Ajouter" />
                            &nbsp;&nbsp;
                            <input type="button" name="del_user" id="del_user" onclick="delUser()" value="Supprimer" style="display:none;" />
                        </td>
                    </tr>
                </table>
            </form>
        ...
        <div class="tag_cloud">
            <form name="money" action="/?act=set_setting" method="post">
                ...
                <input type="text" name="value" size="4" value="{MONEY}" />&nbsp;<input type="submit" name="submit" value="Ok" />
            </form>
        </div>
    ...
    <div class="maincontent">
        <div class="tag_cloud" id="list_groups">
            <!-- BEGIN groups -->
            <div id="group{groups.ID_GROUP}" onclick="getGroup({groups.ID_GROUP})">{groups.NAME} - {groups.COUNT} joueur(s)</div>
            <!-- END groups -->
        </div>
        ...
        <div class="tag_cloud">
            <form name="add_group" action="/?act=add_group" method="post">
                ...
                <table>
                    ...
                    <tr><td><input type="text" size="25" name="group_name" id="group_name" /></td></tr>
                    <tr>
                        <td style="text-align:center;">
                            <input type="button" name="add_group" id="add_group" onclick="saveGroup()" value="Ajouter" />
                            <input type="button" name="del_group" id="del_group" onclick="deleteGroup()" value="Supprimer" style="display:none;" />
                        </td>
                    </tr>
                </table>
            </form>
