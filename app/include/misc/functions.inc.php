...
function cut_title($title, $lenght_max) {
    ...
    // First word too long
    if (strlen($words[0]) > $cut_lenght)
        return substr($words[0], 0, $cut_lenght) . "...";
    else
    ...
    for ($i = 1; $i < $num_words; $i++) {
        ...
        // String too long
        if (($current_lenght > $cut_lenght) && ($previous_string != ""))
...
function FirstPost($chaine) {
    ...
    if (strstr($chaine, "firstpost"))
        return true;
    if (strstr($chaine, "1stpost"))
        return true;
    return false;
}
        ...
        foreach ($a['matchs'] as $match) {
            if ($match['teamBid'] == $b['teamID']) {
                if ($match['scoreMatchA'] > $match['scoreMatchB'])
                    return -1;
                elseif ($match['scoreMatchA'] < $match['scoreMatchB'])
                    return 1;
                else {
                    if (($match['pnyMatchA'] != NULL) && ($match['pnyMatchB'] != NULL)) {
                        return ($match['pnyMatchA'] > $match['pnyMatchB']) ? -1 : 1;
                    }
                }
...
function compare_pronoteams_1to1($a, $b) {
    if ($a['points'] == $b['points']) {
        foreach ($a['matchs'] as $match) {
            if ($match['teamBid'] == $b['teamID']) {
                if ($match['scorePronoA'] > $match['scorePronoB'])
                    return -1;
                elseif ($match['scorePronoA'] < $match['scorePronoB'])
                    return 1;
                else {
                    if (($match['pnyPronoA'] != NULL) && ($match['pnyPronoB'] != NULL)) {
                        return ($match['pnyPronoA'] > $match['pnyPronoB']) ? -1 : 1;
                    }
                }
            } elseif ($match['teamAid'] == $b['teamID']) {
                if ($match['scorePronoA'] > $match['scorePronoB'])
...
function prepare_numeric_data($args) {
    for ($i = 0; $i < count($args); $i++) {
        if (!is_numeric($args[$i]) || $args[$i] == "" || $args[$i] == null)
            $args[$i] = 'NULL';
    }
}
